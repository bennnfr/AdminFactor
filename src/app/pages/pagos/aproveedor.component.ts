...
export class AproveedorComponent implements OnInit {
  ...
  muestraFacturas() {
    ...
    this._pagosservice.getFacturasPagoProveedor( this.idp, valormoneda ).subscribe( resp => { this.facturas = resp;
  ...
  guardar() {
    ...
    if (this.selectedFac.length === 0) {
      ...
      const params = {
        token: '',
        secret_key: '',
        payment_date: (document.getElementById('fecha') as HTMLInputElement).value,
