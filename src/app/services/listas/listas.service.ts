...
export class ListasService {
  ...
  crearLista( dominio: string, key: string, valor: string, descripcion: string ) {
    ...
    const url = `${URL_SERVICIOS}/lists?list[domain]=${dominio}&list[key]=${key}&list[value]=${valor}&token=${this.token}&secret_key=${SECRET_KEY}&list[description]=${descripcion}`;
  ...
  getListas() {
    ...
    const url = `${URL_SERVICIOS}/lists?token=${this.token}&secret_key=${SECRET_KEY}`;
