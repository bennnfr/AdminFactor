...
export class OptionsService {
  ...
  crearOption( name: string, desc: string, group: string, urlo: string ) {
    ...
    const url = `${ URL_SERVICIOS }/options?option[name]=${ name }&option[description]=${ desc }&option[group]=${ group }&option[url]=${ urlo }&token=${ this.token }&secret_key=${ SECRET_KEY }`;
...
getOption( id: string ) {
  ...
  const url = `${URL_SERVICIOS}/options/${id}?token=${this.token}&secret_key=${SECRET_KEY}`;
...
const url = `${URL_SERVICIOS}/options/${ido}?token=${this.token}&secret_key=${SECRET_KEY}&option[name]=${nombre}&option[description]=${desc}&option[group]=${group}&option[url]=${urlo}`;
